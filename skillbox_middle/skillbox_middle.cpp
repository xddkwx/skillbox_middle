...
public:
    ...
    // * Перезагрузка оператора умножения (на число)

    friend Vector operator*(const Vector& a, const float& b);
    friend Vector operator*(const float& a, const Vector& b);

    // * Перезагрузка оператора разности (на вектор)

    friend Vector operator-(const Vector& a, const Vector& b);

    // * Перезагрузка оператора ввода
...
std::istream& operator>>(std::istream& in, Vector& v)
{
    ...
    // если ввод удачный, то присваиваем
    if (in)
    {
        v.setX(x);
        v.setY(y);
